...
def generate_reason(row):
    ...
    # ①日足のテクニカル
    if pd.notna(row['日足RSI']):
        if row['日足RSI'] < 35:
            reasons.append(f"日足RSIが{row['日足RSI']:.1f}と極度の売られすぎ水準です。")
        elif row['日足RSI'] < 45:
            reasons.append(f"日足RSI{row['日足RSI']:.1f}で調整が進み、押し目買い候補です。")
        elif row['日足RSI'] > 70:
            reasons.append(f"日足RSI{row['日足RSI']:.1f}で過熱感あり。短期的な利確目安です。")
    ...
    if st.button("🎯 監視銘柄の最新データを取得"):
        with st.spinner('指定された銘柄のデータを取得中...'):
            ...
            if not df2.empty:
                ...
                df2['日足RSI'] = df2['日足RSI'].round(1)
                df2['週足パフォーマンス(%)'] = df2['週足パフォーマンス(%)'].round(1)
                df2['PER(倍)'] = df2['PER(倍)'].round(1)
